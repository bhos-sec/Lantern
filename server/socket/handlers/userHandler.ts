...
export function registerUserHandlers(socket: Socket, io: Server): void {
  socket.on(SOCKET_EVENTS.SET_NAME, (userName: string) => {
    ...
    socket.emit(SOCKET_EVENTS.NAME_SET_SUCCESS);
    broadcastPresence(io);
  });

  socket.on(SOCKET_EVENTS.TOGGLE_ROOM_VISIBILITY, (showRoom: boolean) => {
